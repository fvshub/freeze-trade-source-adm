...
local fullHeight = UDim2.new(0, 340, 0, 500)
local miniHeight = UDim2.new(0, 340, 0, 44)

local MinBtn = Instance.new("TextButton")
MinBtn.Size = UDim2.new(0, 28, 0, 28)
MinBtn.Position = UDim2.new(1, -36, 0.5, -14)
MinBtn.BackgroundColor3 = Color3.fromRGB(240, 240, 245)
...
MakeCorner(6).Parent = MinBtn
MakeStroke(Color3.fromRGB(220, 220, 225), 1).Parent = MinBtn

MinBtn.MouseEnter:Connect(function()
    Tween(MinBtn, {BackgroundColor3 = Color3.fromRGB(225, 225, 232)})
end)
MinBtn.MouseLeave:Connect(function()
    Tween(MinBtn, {BackgroundColor3 = Color3.fromRGB(240, 240, 245)})
end)
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        ...
        Tween(Main, {Size = miniHeight}, 0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        Log("Panel minimized", "INFO")
    else
        MinBtn.Text = "─"
        Tween(Main, {Size = fullHeight}, 0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
        task.wait(0.18)
        Scroll.Visible = true
        Log("Panel restored", "INFO")
    end
end)
...
local function MakeSection(title, icon, layoutOrder, collapsible)
    ...
    -- Header row (holds title + optional arrow)
    ...
    if collapsible then
        ...
        arrowBtn.Position = UDim2.new(1, -24, 0.5, -12)
        arrowBtn.BackgroundColor3 = Color3.fromRGB(240, 240, 245)
        ...
        MakeCorner(6).Parent = arrowBtn

        arrowBtn.MouseEnter:Connect(function()
            Tween(arrowBtn, {BackgroundColor3 = Color3.fromRGB(225, 225, 232)})
        end)
        arrowBtn.MouseLeave:Connect(function()
            Tween(arrowBtn, {BackgroundColor3 = Color3.fromRGB(240, 240, 245)})
        end)

        arrowBtn.MouseButton1Click:Connect(function()
            sectionExpanded = not sectionExpanded
            arrowBtn.Text = sectionExpanded and "▾" or "▸"
            -- Toggle visibility of all children except the header row itself
...
local targetSection = MakeSection("Select Target", "🎯", 1, true)
